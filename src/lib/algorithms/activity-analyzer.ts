...
export function shouldSuggestRestDay(
  ...
  // High activity + many recent workouts = suggest rest
  ...
  // Elevated resting HR suggests overtraining
...
export function getActivityCalorieAdjustment(
  ...
  if (diff > 3000) return 200; // Very active day
  if (diff > 1500) return 100; // Moderately more active
  if (diff < -3000) return -150; // Very sedentary day
  if (diff < -1500) return -75; // Less active than usual
  return 0;
}
